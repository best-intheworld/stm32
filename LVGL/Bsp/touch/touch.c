...
{
    if (lcddev.dir) {
        *x = touch_readXOY(0x90);
        *y = touch_readXOY(0xD0);
    }
    else {
...
uint8_t touchScan(uint8_t mode)
{
    if (GPIO_ReadInputDataBit(TOUCH_PEN_PORT,TOUCH_PEN_PIN) == 0)     /* 有按键按下 */
    {
        ...
        else if (touch_readXY2(&touch_dev.x[0], &touch_dev.y[0]))     /* 读取屏幕坐标, 需要转换 */
        {
            ...
            touch_dev.x[0] = (signed short)(touch_dev.x[0] - touch_dev.xc) / touch_dev.xfac + lcddev.width / 2;

            /* 将Y轴 物理坐标转换成逻辑坐标(即对应LCD屏幕上面的Y坐标值) */
            touch_dev.y[0] = (signed short)(touch_dev.y[0] - touch_dev.yc) / touch_dev.yfac + lcddev.height / 2;
        }
        ...
        if ((touch_dev.sta & 0xc000) == TP_CATH_PRES)   /* 按键按下了一次(此时按键松开了.) */
        {
            ...
            switch (cnt)
            {
                case 1:
                    ...
                    touch_drawPoint(lcddev.width - 20, 20, RED);    /* 画点2 */
                    break;

                case 2:
                    touch_drawPoint(lcddev.width - 20, 20, WHITE);  /* 清除点2 */
                    touch_drawPoint(20, lcddev.height - 20, RED);   /* 画点3 */
                    break;

                case 3:
                    touch_drawPoint(20, lcddev.height - 20, WHITE); /* 清除点3 */
                    touch_drawPoint(lcddev.width - 20, lcddev.height - 20, RED);    /* 画点4 */
                    break;

                case 4:
                    LCD_clear(WHITE);   /* 画第五个点了, 直接清屏 */
                    touch_drawPoint(lcddev.width / 2, lcddev.height / 2, RED);  /* 画点5 */
                    break;
                ...
                case 5:     /* 全部5个点已经得到 */
                    ...
                    {
                        cnt = 0;
                        touch_drawPoint(lcddev.width / 2, lcddev.height / 2, WHITE); /* 清除点5 */
                        touch_drawPoint(20, 20, RED);   /* 重新画点1 */
                        touch_adjustInfoShow(pxy, px, py);   /* 显示当前信息,方便找问题 */
                        continue;
                    }

                    touch_dev.xfac = (float)(s1 + s3) / (2 * (lcddev.width - 40));
                    touch_dev.yfac = (float)(s2 + s4) / (2 * (lcddev.height - 40));
...
void touch_adjustInfoShow(uint16_t xy[5][2], double px, double py)
{
    ...
    /* 显示X/Y方向的比例因子 */
    LCD_fillRectangle(40, 160 + (i * 20), lcddev.width - 1, 16, WHITE);  /* 清除之前的px,py显示 */
