...
void TOUCH_SCK(uint8_t state)
{
    if(state)
    {
        HAL_GPIO_WritePin(TOUCH_SCK_PORT,TOUCH_SCK_PIN,SET);
    }
    else
    {
        HAL_GPIO_WritePin(TOUCH_SCK_PORT,TOUCH_SCK_PIN,RESET);
    }
}
...
void TOUCH_MOSI(uint8_t state)
{
    if(state)
    {
        HAL_GPIO_WritePin(TOUCH_MOSI_PORT,TOUCH_MOSI_PIN,SET);
    }
    else
    {
        HAL_GPIO_WritePin(TOUCH_MOSI_PORT,TOUCH_MOSI_PIN,RESET);
    }
}
...
void TOUCH_CS(uint8_t state)
{
    if(state)
    {
        HAL_GPIO_WritePin(TOUCH_CS_PORT,TOUCH_CS_PIN,SET);
    }
    else
    {
        HAL_GPIO_WritePin(TOUCH_CS_PORT,TOUCH_CS_PIN,RESET);
    }
}
...
{
    if (HAL_GPIO_ReadPin(TOUCH_PEN_PORT,TOUCH_PEN_PIN) == 0)     /* 有按键按下 */
    {
        if (mode)       /* 读取物理坐标, 无需转换 */
        {
...
void touchAdjust(void)
{
    ...
    LCD_clear(WHITE);       /* 清屏 */
    LCD_showString(40, 40, 160, 100, 16, TP_REMIND_MSG_TBL, RED,WHITE); /* 显示提示信息 */
        ...
        if ((touch_dev.sta & 0xc000) == TP_CATH_PRES)   /* 按键按下了一次(此时按键松开了.) */
        {
            ...
            switch (cnt)
            {
                ...
                case 5:     /* 全部5个点已经得到 */
                    ...
                    LCD_showNum(160,0,s1,5,16,RED,WHITE);
                    LCD_showNum(160,17,s2,5,16,RED,WHITE);
                    LCD_showNum(160,34,s3,5,16,RED,WHITE);
                    LCD_showNum(160,52,s4,5,16,RED,WHITE);
                    px = (double)s1 / s3;       /* X轴比例因子 */
                    py = (double)s2 / s4;       /* Y轴比例因子 */
                    LCD_showNum(160,100,(double)px,5,16,RED,WHITE);
                    LCD_showNum(160,120,py,5,16,RED,WHITE);
                    if (px < 0)px = -px;        /* 负数改正数 */
                    if (py < 0)py = -py;        /* 负数改正数 */
                    ...
                    LCD_clear(WHITE);   /* 清屏 */
                    LCD_showString(35, 110, width, length, 16, "Touch Screen Adjust OK!", BLUE,WHITE); /* 校正完成 */
                    delay_ms(1000);
                    touch_saveAdjustData();
...
void touch_adjustInfoShow(uint16_t xy[5][2], double px, double py)
{
    ...
    for (i = 0; i < 5; i++)   /* 显示5个物理坐标值 */
    {
        ...
        LCD_showString(40, 160 + (i * 20), width, length, 16, sbuf, RED,WHITE);
        sprintf(sbuf, "y%d:%d", i + 1, xy[i][1]);
        LCD_showString(40 + 80, 160 + (i * 20), width, length, 16, sbuf, RED,WHITE);
    }

    /* 显示X/Y方向的比例因子 */
    LCD_fill(40, 160 + (i * 20), width - 1, 16, WHITE);  /* 清除之前的px,py显示 */
