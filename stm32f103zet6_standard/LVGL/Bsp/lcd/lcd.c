...
{
    uint32_t total = width * length;
    LCD_setDisplayAera(0, 239, 0, 319);
    LCD_prepareWriteRam();
...
{
    LCD_writeCommand(setXCmd);
    LCD_writeData(x >> 8);
    LCD_writeData(x & 0xFF);
    LCD_writeCommand(setYCmd);
    LCD_writeData(y >> 8);
